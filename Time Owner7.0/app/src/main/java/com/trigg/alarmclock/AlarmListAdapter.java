...
public class AlarmListAdapter extends BaseAdapter {
	...
	public AlarmListAdapter(Context context, List<AlarmModel> alarms, FirstFragment fragment) {
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		TextView txtEndTime = (TextView) view.findViewById(R.id.alarm_item_timeend);
		txtEndTime.setText(String.format("%02d : %02d", model.timeHourend, model.timeMinuteend));
		...
		btnToggle.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ((FirstFragment) fragment).setAlarmEnabled(((Long) buttonView.getTag()).longValue(), isChecked);
			}
		});
		...
		view.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				((FirstFragment) fragment).startAlarmDetailsActivity(((Long) view.getTag()).longValue());
			}
		});
		...
		view.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View view) {
                ((FirstFragment) fragment).deleteAlarm(((Long) view.getTag()).longValue());
				return true;
			}
		});
	...
	private void updateTextColor(TextView view, boolean isOn) {
		if (isOn) {
			view.setTextColor(Color.rgb(0,122,122));
		} else {
